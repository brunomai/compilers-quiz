...
Essa etapa também captura informações sobre o programa fonte para que as fases subsequentes gerem o código objeto, um importante componente da analise semântica é a verificação de tipos, nela o compilador verifica se cada operador recebe os operandos permitidos e especificados na linguagem fonte.__
__São principais erros são no escopo das declarações de variáveis e classes, incompatibilidade de tipos, identificar declarações com nomes repetidos e uso de controles de fluxos incorretos (continue, break, ttsbegin sem o ttscommit e etc.).__~Bruno N.
