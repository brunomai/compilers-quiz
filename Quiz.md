...
__R:Análise Léxica:A análise léxica também conhecida como scanner é a primeira fase de um processo de compilação e sua função é fazer a leitura do programa fonte, caractere a caractere, agrupar os caracteres em lexemas e produzir uma sequência de símbolos léxicos conhecidos como tokens. Pode ser dividida em duas etapas, a primeira chamada de escandimento que é uma simples varredura removendo comentários e espaços em branco, e a segunda etapa, a analise léxica propriamente dita onde o texto é quebrado em lexemas, a formação de uma lexema se dá a partir do primeiro caractere do código fonte ou no próximo caractere do último lexema formado, analisa caractere a caractere até encontrar um caractere de parada, caso encontre um caractere que não pertence a linguagem a compilação é parada e um erro é gerado, se todos os caracteres pertencerem a gramatica um lexema é formado, no fim da analise uma tabela de tokens com as lexemas é gerada.__~Bruno N.

__Análise Sintática:A sintaxe também conhecido como parser é a segunda etapa de compilação e a parte da gramática que estuda a disposição das palavras na frase e das frases em um discurso. Essa etapa no processo de compilação deve reconhecer a forma do programa fonte e determinar se ele é valido ou não. Esse modelo pode ser definido utilizando gramáticas livres de contexto que representam uma gramática formal e pode ser escrita através de algoritmos fazendo a derivação de todas as possíveis construções da linguagem, ou seja, analisa os lexemas em suas determinadas posições na instrução e caso aja alguma incoerência dispara um erro, por exemplo, se tiver um for no lugar de um if, ou uma palavra reservada no lugar de uma variável e por fim monta uma arvore sintática.__~Bruno N.

__Análise semântica:A análise semântica é responsável por verificar aspectos relacionados ao significado das instruções, essa é a terceira etapa do processo de compilação e nesse momento ocorre a validação de uma serie regras que não podem ser verificadas nas etapas anteriores a fim de garantir que o programa fonte esteja coerente e o mesmo possa ser convertido para linguagem de máquina.
A análise semântica percorre a árvore sintática relaciona os identificadores com seus dependentes de acordo com a estrutura hierárquica.
Essa etapa também captura informações sobre o programa fonte para que as fases subsequentes gerem o código objeto, um importante componente da analise semântica é a verificação de tipos, nela o compilador verifica se cada operador recebe os operandos permitidos e especificados na linguagem fonte.São principais erros são no escopo das declarações de variáveis e classes, incompatibilidade de tipos, identificar declarações com nomes repetidos e uso de controles de fluxos incorretos (continue, break, ttsbegin sem o ttscommit e etc.).__~Bruno N.
